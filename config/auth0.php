 ...
 */
return [
    // Should be assigned either 'api', 'management', or 'webapp' to indicate your application's use
    // case for the SDK.
    ...
    // One or more API identifiers, found in your Auth0 API settings. The SDK uses the first value
    // for building links. If provided, at least one of these values must match the 'aud' claim to
    // validate an ID Token successfully.
    ...
    // One or more Organization IDs, found in your Auth0 Organization settings. The SDK uses the
    // first value for building links. If provided, at least one of these values must match the
    // 'org_id' claim to validate an ID Token successfully.
    'organization' => \Auth0\Laravel\Configuration::stringToArrayOrNull(env('AUTH0_ORGANIZATION')),

    // The secret used to derive an encryption key for the user identity in a session cookie and to
    // sign the transient cookies used by the login callback.
    'cookieSecret' => env('AUTH0_COOKIE_SECRET', env('APP_KEY')),

    // How long, in seconds, before cookies expire. If set to 0 the cookie will expire at the end of
    // the session (when the browser closes).
    'cookieExpires' => env('COOKIE_EXPIRES', 0),

    // Cookie domain, for example 'www.example.com', for use with PHP sessions and SDK cookies.
    // Defaults to value of HTTP_HOST server environment information.
    // Note: To make cookies visible on all subdomains then the domain must be prefixed with a dot
    // like '.example.com'.
    'cookieDomain' => env('AUTH0_COOKIE_DOMAIN'),

    // Specifies path on the domain where the cookies will work. Defaults to '/'. Use a single slash
    ...
    // Named routes within your Laravel application that the SDK may call during stateful requests
    // for redirections.
    'routes' => [
